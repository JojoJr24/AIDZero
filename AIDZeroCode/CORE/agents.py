...
def resolve_agents_root(repo_root: Path) -> Path:
    """Resolve canonical Agents directory from current runtime root."""

    root = repo_root.resolve()
    local_agents = (root / "Agents").resolve()
    if root.name == "AIDZeroCode":
        parent_agents = (root.parent / "Agents").resolve()
        if parent_agents.is_dir() or not local_agents.exists():
...
class AgentProfileManager:
    ...
    def __init__(self, repo_root: Path) -> None:
        ...
        # Resolve symlinks so path safety checks remain valid after repo refactors.
        self.profiles_root = resolve_agents_root(self.repo_root)
