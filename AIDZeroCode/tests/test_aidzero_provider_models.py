...
def test_list_provider_models_reads_nested_aidzero_code_layout(tmp_path: Path) -> None:
    _write(
        tmp_path / "AIDZeroCode" / "LLMProviders" / "demo" / "provider.py",
        "class DemoProvider:\n"
        "    def list_model_names(self):\n"
        "        return ['model-z']\n",
    )
